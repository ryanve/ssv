...
ok(ssv.need("", "") === "")
ok(ssv.need("", "tom") === "tom")
ok(ssv.need("tom tom", "") === "tom tom")
ok(ssv.need("mark tom", "mark") === "mark tom")
ok(ssv.need("mark tom", "mark mark") === "mark tom")
ok(ssv.need("mark tom", "travis") === "mark tom travis")
ok(ssv.need("mark tom travis", "travis") === "mark tom travis")
ok(ssv.need("mark tom", "travis travis") === "mark tom travis travis")
ok(ssv.need("mark tom tom", "travis") === "mark tom tom travis")
ok(ssv.need("  mark  tom  ", "  travis  ") === "mark tom travis")
ok(ssv.need("  mark  tom  ", "tom") === "mark tom")
ok(ssv.need("mark mark", "tom tom") === "mark mark tom tom")
ok(ssv.need(0, 182) === "0 182")
ok(ssv.need(182, 0) === "182 0")
...
ok(ssv() instanceof ssv)
ok(new ssv instanceof ssv)
ok(ssv.call() instanceof ssv)
ok(ssv().$ === "")
ok(ssv(undefined).$ === "")
ok(ssv(null).$ === "")
ok(ssv("182").$ === "182")
ok(ssv(182).$ === "182")
ok(ssv().hasOwnProperty("$"))
ok(ssv().constructor === ssv)
console.log("constructor tests passed")

var mark = ssv("mark")
var nark = ssv()
ok(mark.diff("mark") !== mark)
ok(ssv.call(nark) !== nark)
console.log("immutable tests passed")

ok(String(ssv(182)) === "182")
ok(ssv(182).toString() === "182")
ok(ssv.prototype.toString() === "")
console.log("toString tests passed")

ok(ssv.prototype.count() === 0)
ok(ssv.prototype.compact() instanceof ssv)
ok(ssv.prototype.compact().$ === "")
ok(ssv.prototype.concat("tom").$ === "tom")
ok(ssv.prototype.split() instanceof Array)
ok(ssv.prototype.split().length === 0)
...
ok(ssv(true).$ === "true")
ok(ssv(false).$ === "false")
ok(ssv("mark tom scott").any("mark"))
ok(ssv("mark tom scott").count() === 3)
ok(ssv("mark tom scott").diff("scott").count() === 2)
ok(ssv("mark tom scott").diff("scott").$ === "mark tom")
ok(
  ssv("mark tom scott")
    .diff("scott scott")
    .union("travis travis")
